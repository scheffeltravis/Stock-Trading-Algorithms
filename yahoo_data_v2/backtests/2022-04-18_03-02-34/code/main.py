...
class FirstAlgorithm(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        for i in range(len(self.ma_lengths)):
            ...
            if data['TQQQ'].Close > int(self.moving_averages_high[i].Current.Value) and self.trade_status[i] == 0:
                self.trade_status[i] = 1
                changed = True

            if data['TQQQ'].Close < int(self.moving_averages_low[i].Current.Value) and self.trade_status[i] == 1:
